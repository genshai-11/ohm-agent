...
import { AgentConfig, AgentRequest, AgentResponse, LABEL_COLORS, LlmProvider, ModelOption } from './types.ts';
import { evaluateTranscript } from './services/agentService.ts';
import { fetchProviderModels } from './services/providerConfigService.ts';
...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<'evaluator' | 'admin'>('evaluator');
  ...
  const loadModels = useCallback(async (selectedProvider: LlmProvider) => {
    try {
      ...
      console.warn('Failed to load models, fallback to current model', err);
    }
  }, []);
  ...
  }, [transcript, provider, modelId, reactionDelay, sessionId]);

  const handleConfigUpdated = (config: AgentConfig) => {
    if (config.defaultProvider) {
      setProvider(config.defaultProvider);
      const defaultModel = config.providers?.[config.defaultProvider]?.model;
      if (defaultModel) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex items-center justify-between gap-3 pb-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Provider-ready evaluator + admin config (v1)</p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm ${viewMode === 'evaluator' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
            >
              Evaluator
            </button>
            <button
              onClick={() => setViewMode('admin')}
              className={`px-3 py-2 rounded-lg text-sm ${viewMode === 'admin' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
            >
              Admin
            </button>
          </div>
        ...
        {viewMode === 'admin' ? (
          <AdminConfigPage onConfigUpdated={handleConfigUpdated} />
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-4">
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-semibold flex items-center gap-2">Transcript Input</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <label className="text-xs font-medium text-slate-500">
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      >
                        {(modelOptions.length ? modelOptions : [{ id: modelId, displayName: modelId, provider }]).map((m) => (
                      ...
                      <button
                        ...
                        className="px-2 py-2 rounded bg-slate-100 hover:bg-slate-200 text-xs"
                      >
                        Fetch
                      </button>
                    </div>
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="text-sm font-mono font-medium w-12 text-right">{reactionDelay}</span>
                </div>
                        ...
                        {result.chunks.map((chunk, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex items-start gap-3 p-3 rounded-lg border border-slate-100 bg-white">
                              ...
                              <div className="ml-auto text-xs font-mono text-slate-400">{(chunk.confidence * 100).toFixed(0)}% conf</div>
                            </div>
                          );
                        })}
                      </div>
